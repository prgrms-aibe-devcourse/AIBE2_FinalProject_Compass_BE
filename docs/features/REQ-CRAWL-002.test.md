...
- [ ] **케이스 1**: 전체 지역 크롤링 시작
    ...
    - **예상 결과**: HTTP Status `200 OK`가 반환되고, 크롤링이 시작되었다는 메시지가 포함됩니다.
    - **설명**: 서울, 부산, 제주 지역의 모든 카테고리 크롤링이 시작되는지 검증합니다.

- [ ] **케이스 2**: 특정 지역 크롤링 시작
    - **입력**: `POST /api/crawl/start/1` 요청 (서울)
    - **예상 결과**: HTTP Status `200 OK`가 반환되고, 서울 지역 크롤링이 시작되었다는 메시지가 포함됩니다.
    - **설명**: 특정 지역의 크롤링이 정상적으로 시작되는지 검증합니다.

- [ ] **케이스 3**: 크롤링 진행 상황 조회
    - **입력**: `GET /api/crawl/status` 요청
    - **예상 결과**: HTTP Status `200 OK`가 반환되고, 전체 크롤링 통계와 진행 상황이 포함됩니다.
    - **설명**: 크롤링 진행 상황이 정확히 조회되는지 검증합니다.

- [ ] **케이스 4**: 특정 지역 크롤링 상태 조회
    - **입력**: `GET /api/crawl/status/1` 요청 (서울)
    - **예상 결과**: HTTP Status `200 OK`가 반환되고, 서울 지역의 크롤링 상태가 포함됩니다.
    - **설명**: 특정 지역의 크롤링 상태가 정확히 조회되는지 검증합니다.

- [ ] **케이스 5**: 크롤링 결과 통계 조회
    - **입력**: `GET /api/crawl/results` 요청
    - **예상 결과**: HTTP Status `200 OK`가 반환되고, 수집된 데이터의 통계 정보가 포함됩니다.
    - **설명**: 크롤링 결과 통계가 정확히 조회되는지 검증합니다.
...
spring:
  datasource:
    url: jdbc:postgresql://localhost:5432/compass_db
...
### 크롤링 성능 예상
- **서울 지역**: 약 1,600개 데이터 - 약 15분
- **부산 지역**: 약 1,130개 데이터 - 약 10분
- **제주 지역**: 약 1,170개 데이터 - 약 10분
- **전체 크롤링**: 약 3,900개 데이터 - 약 35분
...
- [ ] TourPlace 엔티티 및 테이블 생성 성공
- [ ] CrawlStatus 엔티티 및 테이블 생성 성공
- [ ] 크롤링 서비스 구현 및 테스트 성공
- [ ] 크롤링 컨트롤러 구현 및 테스트 성공
- [ ] 서울 지역 크롤링 성공 (1,600개 예상)
- [ ] 부산 지역 크롤링 성공 (1,130개 예상)
- [ ] 제주 지역 크롤링 성공 (1,170개 예상)
- [ ] 크롤링 상태 모니터링 기능 정상 동작
- [ ] 데이터베이스 저장 검증 완료
- [ ] 전체 크롤링 통계 조회 성공
...
- **📅 최종 검증**: 2025-09-10 16:30 - Phase별 크롤링 테스트 완료
