...
public class IntentClassifier {
    ...
    private String createSystemPrompt() {
        return """
            당신은 여행 챗봇의 의도 분류 전문가입니다.
            목표: 모든 대화를 결국 여행 계획 생성으로 자연스럽게 유도하기 위한 분류입니다.

            사용자 메시지를 분석하여 다음 3가지 중 하나로 분류하세요:

            1. TRAVEL_INFO_COLLECTION: 사용자가 여행 계획을 짜달라고 명시적으로 요청하는 경우
               예: "여행 계획 짜줘", "일정 짜줘", "3박 4일 제주도 여행 계획", "여행 가고 싶어"
               * 이 분류는 정보 수집 Phase로 진입하는 트리거입니다.

            2. TRAVEL_QUESTION: 여행 관련 정보를 묻는 질문 (여행 계획으로 유도 가능)
               예: "파리 날씨 어때?", "제주도 맛집 추천", "비자 필요해?", "호텔 추천"
               * 이런 질문에 답하면서 여행 계획으로 자연스럽게 유도합니다.

            3. GENERAL_CHAT: 여행과 무관한 일반 대화 (여행 화제로 전환 필요)
               예: "안녕하세요", "고마워", "날씨 좋네", "뭐해?"
               * 일반 대화에서도 여행 화제로 전환할 기회를 찾습니다.

            반드시 위 3가지 중 하나만 응답하세요.

            중요: 사용자가 여행에 대한 관심을 조금이라도 보이면 TRAVEL_QUESTION으로 분류하여
                  여행 계획으로 자연스럽게 이어질 수 있도록 합니다.
            """;
    }
    ...
    private String createUserPrompt(String message) {
        return String.format("""
            ...
            분류 시 고려사항:
            - 여행 키워드가 있으면 TRAVEL_QUESTION으로 분류
            - 명확한 계획 요청만 TRAVEL_INFO_COLLECTION으로 분류
            - 그 외는 GENERAL_CHAT으로 분류
