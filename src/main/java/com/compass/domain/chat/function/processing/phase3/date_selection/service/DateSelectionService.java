...
public class DateSelectionService {
    ...
    public DateSelectionOutput processDistribution(String threadId, int tripDays) {
        ...
        try {
            log.info("날짜별 선별 처리 시작");

            var destinationResult = loadDestinationData(threadId, tripDays);
            var dailyItineraries = processItineraries(destinationResult.places(), tripDays);
    ...
    // 여행지 선별 데이터 로드
    private DestinationSelectionResult loadDestinationData(String threadId, int tripDays) {
        log.debug("여행지 선별 데이터 로드 중");

        // Stage 1에서 데이터를 받아와야 함 - 현재는 빈 결과 반환
        // TODO: Stage 1 구현 후 실제 데이터 로드 로직 추가
        var places = new ArrayList<TourPlace>();
        var result = new DestinationSelectionResult(
            "dest-" + threadId,
            threadId,
            places,
        ...
        log.info("여행지 선별 데이터 로드 완료: {}개 장소", result.places().size());
        return result;
    }
    ...
    // 최종 출력 생성
    private DateSelectionOutput createOutput(Map<Integer, DailyItinerary> itineraries) {
