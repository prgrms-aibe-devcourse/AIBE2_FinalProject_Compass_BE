...
public class ChatInputParser {
    ...
    private static final Pattern LOCATION_PATTERN = Pattern.compile(
        "(서울|부산|제주|경주|강릉|전주|여수|춘천|속초|대구|대전|광주|인천)|" +
        "([가-힣]+시)|([가-힣]+도)"
    );
    ...
    private void extractDates(String input, TripPlanningRequest request) {
        ...
        // Handle duration patterns (e.g., "3박4일")
        if (input.contains("박") && input.contains("일")) {
            Pattern durationPattern = Pattern.compile("(\\d+)박\\s?(\\d+)일");
            Matcher durationMatcher = durationPattern.matcher(input);
            if (durationMatcher.find()) {
                int nights = Integer.parseInt(durationMatcher.group(1));
    ...
    private Integer parseBudget(String budgetStr) {
        try {
            ...
            String cleaned = budgetStr.replaceAll("[,\\s]", "");
            
            // Extract number - look for number before unit
            Pattern numPattern = Pattern.compile("(\\d+)(만원|천원|원|달러|USD)?");
            ...
            if (numMatcher.find()) {
                ...
                // If just "원" or no unit specified, assume it's already in KRW
                return amount;
            }
