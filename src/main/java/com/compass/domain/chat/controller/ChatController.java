package com.compass.domain.chat.controller;
  ...
  @Tag(name = "Chat", description = "Chat API for AI model 
  interactions")
  public class ChatController {
      ...
      public ChatController(
              ...
              @Qualifier("geminiChatService") ChatModelService 
  geminiChatService,
              @Qualifier("openAIChatService") ChatModelService 
  openAiChatService,
      ...
      public ResponseEntity<ChatDtos.ThreadDto> createChatThread(
          ...
          ChatDtos.ThreadDto newThread =
  chatService.createThread(userId);
      ...
      public ResponseEntity<List<ChatDtos.ThreadDto>> getChatThreads(
          ...
          List<ChatDtos.ThreadDto> threads =
  chatService.getUserThreads(userId, skip, limit);
  ...
  {
          List<ChatDtos.MessageDto> messages =
  ...
  ResponseEntity.status(HttpStatus.NOT_FOUND).body(null);
      ...
      public ResponseEntity<List<ChatDtos.MessageDto>>
  getMessagesInThread(
              @PathVariable String threadId,
              @RequestHeader("X-User-ID") String userId,
          ...
          List<ChatDtos.MessageDto> messages =
  chatService.getMessages(threadId, userId, limit, before);
          if (messages == null) {
              return
  ResponseEntity.status(HttpStatus.NOT_FOUND).body(null);
          }
      ...
      @Operation(summary = "Send message to Gemini")
      @PostMapping("/gemini")
      public ResponseEntity<Map<String, Object>> chatWithGemini(
          ...
          try {
              String response =
  geminiChatService.generateResponse(message);
      ...
      @Operation(summary = "Send message to OpenAI")
      @PostMapping("/openai")
  ...
  openAiChatService.generateResponse(message);
      ...
      public ResponseEntity<Map<String, Object>> chatWithTemplate(
          ...
          try {
              String templateName =
  promptTemplateService.selectTemplate(message, request);
              log.info("Selected template: {} for message: {}",
  templateName, message);

              Map<String, Object> parameters =
  promptTemplateService.extractParameters(
                  templateName, message, request);

              PromptRequest promptRequest = new PromptRequest();
              promptRequest.setTemplateName(templateName);
              promptRequest.setParameters(parameters);
              promptRequest.setUserId(userId != null ? userId.toString()
   : null);

              PromptResponse promptResponse =
  promptTemplateService.buildEnrichedPrompt(promptRequest);
              String aiResponse =
  geminiChatService.generateResponse(promptResponse.getPrompt());
      ...
      public ResponseEntity<Map<String, Object>> getTemplates() {
          ...
          response.put("templates",
  promptTemplateService.getAvailableTemplates());
          response.put("count",
  promptTemplateService.getAvailableTemplates().size());
      ...
      public ResponseEntity<Map<String, Object>> getTemplateDetails(
          ...
          if (!promptTemplateService.getAvailableTemplates().contains(te
  mplateName)) {
              Map<String, Object> error = new HashMap<>();
              error.put("error", "Template not found: " + templateName);
              return ResponseEntity.notFound().build();
          }
          return ResponseEntity.ok(promptTemplateService.getTemplateDeta
  ils(templateName));
      }
